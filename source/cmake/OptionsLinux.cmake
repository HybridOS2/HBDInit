...
CALCULATE_LIBRARY_VERSIONS_FROM_LIBTOOL_TRIPLE(HBDINIT 0 0 0)

# These are shared variables, but we special case their definition so that we can use the
# CMAKE_INSTALL_* variables that are populated by the GNUInstallDirs macro.
set(LIB_INSTALL_DIR "${CMAKE_INSTALL_FULL_LIBDIR}" CACHE PATH "Absolute path to library installation directory")
set(EXEC_INSTALL_DIR "${CMAKE_INSTALL_FULL_BINDIR}" CACHE PATH "Absolute path to executable installation directory")
set(LIBEXEC_INSTALL_DIR "${CMAKE_INSTALL_FULL_LIBEXECDIR}/hbdinit" CACHE PATH "Absolute path to install executables executed by the library")
set(HEADER_INSTALL_DIR "${CMAKE_INSTALL_INCLUDEDIR}" CACHE PATH "Absolute path to header installation directory")
set(HBDINIT_HEADER_INSTALL_DIR "${CMAKE_INSTALL_INCLUDEDIR}/hbdinit" CACHE PATH "Absolute path to HBDInit header installation directory")

add_definitions(-DBUILDING_LINUX__=1)
add_definitions(-DHBDINIT_API_VERSION_STRING="${HBDINIT_API_VERSION}")
...
# and the option is not relevant to any other HBDInit ports.
#HBDINIT_OPTION_DEFINE(USE_SYSTEMD "Whether to enable journald logging" PUBLIC ON)

# Private options specific to the HybridOS port. Changing these options is
# completely unsupported. They are intended for use only by HBDInit developers.
#HBDINIT_OPTION_DEFINE(USE_ANGLE_WEBGL "Whether to use ANGLE as WebGL backend." PRIVATE OFF)
#HBDINIT_OPTION_DEPEND(ENABLE_WEBGL ENABLE_GRAPHICS_CONTEXT_GL)

HBDINIT_OPTION_DEFAULT_PORT_VALUE(ENABLE_XML PUBLIC ${ENABLE_XML_DEFAULT})
HBDINIT_OPTION_DEFAULT_PORT_VALUE(ENABLE_HTTP PUBLIC ${ENABLE_HTTP_DEFAULT})
HBDINIT_OPTION_DEFAULT_PORT_VALUE(ENABLE_LSQL PUBLIC ${ENABLE_LSQL_DEFAULT})
HBDINIT_OPTION_DEFAULT_PORT_VALUE(ENABLE_RSQL PUBLIC ${ENABLE_RSQL_DEFAULT})
HBDINIT_OPTION_DEFAULT_PORT_VALUE(ENABLE_HIBUS PUBLIC ${ENABLE_HIBUS_DEFAULT})
HBDINIT_OPTION_DEFAULT_PORT_VALUE(ENABLE_SSL PUBLIC ${ENABLE_SSL_DEFAULT})
